...
export class OrdersService {
  ...
  getByUniqueId(id:number) : Observable<Order> {
   ...
   getOrderedMedicationByIdentifier(id:number) : Observable<MedicationOrder[]> {
    return this.http.get<MedicationOrder[]>(`${environment.baseUrl}/${environment.order}/${environment.getOrderedMedicationByIdentifier}?id=${id}`);
   }
